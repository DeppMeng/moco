...
def bufread(filename):
    ...
    path_buf = path[pos_at + 2:]
    if not os.path.isfile(path_zip):
        print("zip file '%s' is not found"%(path_zip))
        assert 0
    for i in range(len(_buf_zfile)):
        if _buf_zfile[i]['path'] == path_zip:
            data = _buf_zfile[i]['zipfile'].read(path_buf)
            return data
    ...
    data = _buf_zfile[-1]['zipfile'].read(path_buf)
